...
def main(SETTINGS_JSON):
    ...
    index_old_tensor = settings['currents'] + [len_old_currents,len_old_currents+1,len_old_currents+2]
    print(index_old_tensor)

    dsm_dir_list = [sample for sample in os.listdir(settings["dsm_old_datapath"]) if sample.endswith("dsm_cnn.npy")]
    
    for sample in dsm_dir_list:
        sample_path = os.path.join(settings["dsm_old_datapath"], sample)
        sample_data = np.load(sample_path)

        ## carrega correntes correspondentes 
        ...
        np.save(new_sample_name, sample_data[index_old_tensor])
