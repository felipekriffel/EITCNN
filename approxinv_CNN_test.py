...
def main(RESULTS_PATH):
  ...
  if not os.path.isdir(f"{RESULTS_PATH}/approxinvs"):
    os.mkdir(f"{RESULTS_PATH}/approxinvs")
  ...
  for sample in range(n_exper):
    ...
    T = np.zeros((k_len + 2,N,N))

    for j, k in enumerate(k_list):
      gamma_k.x.array[:] = approxinv.evaluate_method(W_dict[str(k)],b_dict[str(k)],x0_dict[str(k)],U1_array, 'log',lw_clip=1)
      T[j] = eitx.genGammaImg(gamma_k,mesh_x,mesh_y,bg,ivhigh,ivlow,method='default')

      # fig, ax = plt.subplots(1, k_len,figsize = (10*k_len,10))
      # for j in range(k_len):
      #     ax[j].imshow(T[j])
      # plt.savefig(f"{RESULTS_PATH}/approxinvs/{exper[sample]}.png")
