...
def main(SETTINGS_JSON):
    ...
    with open(settings['savepath']+"/data_info.json","w") as f:
    ...
    #Solving Forward Problem for gamma0
    ...
    ## Approximate inverse settings
    alpha_list = settings['alpha_list']
    k_list = settings['k_list']
    k_len = len(k_list)

    alpha = alpha_list[0]

    gamma_k = dolfinx.fem.Function(V0)
    ...
    for k in k_list:
        W_dict[str(k)] = np.load(f'approxinv_data/matrix/W_{k}_alpha_{alpha}.npy')
        b_dict[str(k)] = np.load(f'approxinv_data/matrix/b_{k}_alpha_{alpha}.npy')
        x0_dict[str(k)] = np.load(f'approxinv_data/matrix/x0_{k}_alpha_{alpha}.npy')
    ...
    for m in range(nn_samples):
        ...
        for sample in range(n_samples[m]):
            ...
            list_u, list_U = dir_problem.solve_problem_current(I_all, gamma)

            U_array = np.array(list_U).flatten()

            ## Compute approximate inverses, genareate image in homogeneous grid
            T = np.zeros((k_len + 3,N,N))
            for i, k in enumerate(k_list):
                gamma_k.x.array[:] = approxinv.evaluate_method(W_dict[str(k)],b_dict[str(k)],x0_dict[str(k)],U_array, 'log',lw_clip=1)
                T[i] = eitx.genGammaImg(gamma_k,mesh_x,mesh_y,bg,ivhigh,ivlow,method='default')

            T[k_len] = mesh_x
            T[k_len+1] = mesh_y
            T[k_len+2] = A
            np.save(f"{settings['savepath']}/sample_{m+1}_{sample}",T)
        print('Generation of ' + str(m + 1) + ' circle(s) ended.')
        # np.save('EIT_Data_for_CNN', T1)
    print(f'Data saved at {settings["savepath"]}.')
