...
class DirectProblem:
    def __init__(self, mesh_object: MeshClass):
        ...
        self.bdr_index = dolfinx.fem.locate_dofs_topological(self.V, self.mesh.topology.dim-1, boundary_facets) #array with the vertices index

        #vertices coordinates, on whole domain and boundary
        self.dofs_coordinates = self.V.tabulate_dof_coordinates()
        self.bdr_dofs_coordinates = self.dofs_coordinates[self.bdr_index]

        self.theta = np.where( #theta bdr array, with same order as bdr_index
            self.bdr_dofs_coordinates[:,1]>=0,
            np.arccos(self.bdr_dofs_coordinates[:,0]),
            2*np.pi - np.arccos(self.bdr_dofs_coordinates[:,0])
        )
        ...
        self.mesh_vertex_index_list = mesh_vertex_index_list
        ...
        self.cells_coordinates_list = cells_coordinates_list
    ...
    def set_problem(self, gamma):
        ...
        #A1
        a_eq = (gamma * ufl.inner(gradu, gradv)) * ufl.dx 
        self.A_matrix = dolfinx.fem.petsc.assemble_matrix(dolfinx.fem.form(a_eq))
        
        self.A_matrix.assemble()
        self.A_np = self.A_matrix.getValues(range(self.N),range(self.N))
    ...
    def solve_problem_current(self, I_all,gamma=None,save=False):
        """
        I_all: list of dolfinx functions, with each item being the current to solve for
        gamma: dolfinx function, condductivty. Defaults to self.gamma
        """
        v = ufl.TestFunction(self.V)
        ...
        solver = PETSc.KSP().create(self.mesh.comm) #P().create(domain.comm)
        solver.setOperators(self.A_matrix)
        self.A_matrix.assemble()

        u_sol_list = []
        #Right hand side
        for g in I_all:
            usol = dolfinx.fem.Function(self.V)
            rhs = ufl.inner(g,v) * ufl.ds
            b  = dolfinx.fem.petsc.assemble_vector(dolfinx.fem.form(rhs))
            solver.solve(b, usol.x.petsc_vec)
    ...
    def interpolate_bdr_function(self, fbdr):
        """
        Interpolate boundary function parametrized by angle theta in [0,2pi],
        with an array of function values or,dered same as self.bdr_index. 
        Values only set on bdr, rest of the domain is set to 0

        fbdr: ndarray, function values on self.theta
        """
        f = dolfinx.fem.Function(self.V)
        f.x.array[self.bdr_index] = fbdr
        return f
    
    def compute_f_theta(self,f):
        """
        Gets callable function f parametrized by theta and applies to self.theta, 
        to follow the same order as self.bdr_index.

        """
        return f(self.theta)
    
    def get_current_list(self,n:int,type:str='cos')->list:
        """
        Return list with currents interpolated to the boundary,
        ready to use on `solve_problem_current`

        - n: int, number of currents
        - type:str, type of current list
            - `"cos"`: returns array with (cos(1t),cos(2t),...,cos(nt))
        """
        g_list = []
        if type=="cos":
            for k in range(1,n+1):
                f = lambda t: np.cos(k*t)
                fbdr = self.compute_f_theta(f)
                f_interp = self.interpolate_bdr_function(fbdr)
                g_list.append(f_interp)

        return g_list
    
    def compute_gradient(self,u:dolfinx.fem.Function)->np.array:
        """
        ...
        :param u: Function to compute the gradient (del_x u, del_y u)
        :type u: dolfinx.fem.Function

        :return grad_array: Nx2 nd array, with each gradient component on respective columns
        """
        
        gradient_list = []

        for idx in self.mesh_vertex_index_list:
            cell_coord = self.mesh.geometry.x[idx][:,:2]
    ...
    def adjoint(self, sigma_list, u_list):
        ...
        grad_psi_list = [self.compute_gradient(psi) for psi in psi_list]
        grad_u_list = [self.compute_gradient(u) for u in u_list]
...
class InverseProblem(DirectProblem):
    ...
    def calc_jacobian(self, un_list):
        ...
        grad_y_list = [self.compute_gradient(y_func) for y_func in y_list]
        grad_u_list = [self.compute_gradient(u) for u in un_list]
        l = len(un_list)
