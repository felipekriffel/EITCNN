...
def main(TEST_PATH, RESULTS_PATH):
    ...
    dsm_dir = [file for file in os.listdir(TEST_PATH) if file.endswith("dsm_cnn.npy")]
    ...
    for sample in dsm_dir:
        ...
        T = np.load(sample_path)
    
        input_val = tf.convert_to_tensor([np.transpose(T[:-1])])
    ...
    # for pred, target in zip(pred_array, gammaimg_list):
    #     pred_list.append(
    #         np.array([
    #             pred[:,:,0].T,
    #             target
